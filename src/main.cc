...
static bool probeGenerationModel(const std::string &modelPath) {
    if (gExecutablePath.empty()) return true;
#ifdef _WIN32
    std::string command = quoteArg(gExecutablePath) + " --probe-gen " + quoteArg(modelPath) + " >nul 2>nul";
#else
    std::string command = quoteArg(gExecutablePath) + " --probe-gen " + quoteArg(modelPath) + " >/dev/null 2>&1";
#endif
    int rc = std::system(command.c_str());
    return rc == 0;
}

// 交互式热切换：扫描 models/ 目录列出所有 GGUF → 用户按编号选择生成/Embedding 模型
static bool interactiveReload(AppConfig &activeConfig,
    ...
    if (!input.empty()) {
        ...
        if (idx >= 1 && idx <= genFiles.size()) {
            ...
            if (samePathText(newGenPath, activeConfig.model.gen_model_path)) {
                ...
                std::cout << "[INFO] 正在探测目标模型是否可加载..." << std::endl;
                if (!probeGenerationModel(newGenPath)) {
                    std::cout << "[WARN] 目标生成模型探测未通过，将释放旧模型后再尝试正式加载" << std::endl;
                }
    ...
    (void)embFiles;
    (void)embEngine;
    std::cout << "[INFO] Embedding 模型与当前数据库向量绑定，交互式 /reload 默认不切换 Embedding" << std::endl;
...
static bool reloadFromConfig(AppConfig &activeConfig,
    ...
    if (!sameRuntimeModels(activeConfig, latestConfig)) {
        ...
        if (!samePathText(activeConfig.model.emb_model_path, latestConfig.model.emb_model_path) ||
            ...
            embEngine.reset();
            embEngine = std::make_unique<EmbeddingEngine>(latestConfig.model.emb_model_path,
                                                          latestConfig.embedding, latestConfig.generation,
                                                          latestConfig.model.n_gpu_layers);
        }
...
static void chatLoop(const std::string &dbPath, const AppConfig &config) {
    ...
    while (true) {
        ...
        if (query == "/reload") {
            try {
                interactiveReload(activeConfig, embEngine, genEngine);
        ...
        try {
            reloadFromConfig(activeConfig, embEngine, genEngine);
...
static void doQuery(VectorDatabase &db, EmbeddingEngine &embEngine, GenerationEngine &genEngine,
    ...
    std::cout << "\n[检索到 " << results.size() << " 个相关片段]" << std::endl;
    for (size_t i = 0; i < results.size(); ++i) {
        auto preview = results[i].text.substr(0, std::min<size_t>(80, results[i].text.size()));
